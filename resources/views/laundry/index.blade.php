...
@section('scripts')
    <script type="application/javascript">
        ...
        }//below funtion is used to download data and save in excel format
        function downloadTableAsExcel() {
    // To Fetch table data
    const table = document.querySelector('.laundromat_table');
    ...
    // To Find the index of the "Registered On" column
    const columnHeaderCells = table.querySelector('thead tr').cells;
    const registeredOnColumnIndex = Array.from(columnHeaderCells).findIndex(cell => cell.textContent.trim() === 'Registered On');
    
       
    // To Generate the file name from the "Registered On" field's content
    const datetimeString = tableData[1][registeredOnColumnIndex];
    const datePart = datetimeString.split(' ')[0];
    const excelFileName = datePart.replace(/\//g, '-') + '.xlsx';


    // To Save the workbook as an Excel file
